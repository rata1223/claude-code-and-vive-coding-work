...
- A genuinely new, unrelated-but-relevant finding surfaced during this audit: `api/routers/quick_trade.py`'s `_load_kis` **mutates process-wide `os.environ["KIS_APP_KEY"]` etc. at request time** (`quick_trade.py:22-33`) to inject a per-user decrypted credential before constructing a broker client. This is a latent cross-request credential-bleed risk under concurrent load in a multi-worker server — independent of the INTEGRATE/SEPARATE question, but it further confirms `quick_trade.py` and `backend/execution/*`'s env-var-based single-account model **cannot safely share a process or a credential-provisioning mechanism at all**, reinforcing that they are different architectures by design, not just by historical accident.
...
|---|---|---|
| 변경 파일/모듈 범위 | Large: `backend/database/models.py` (schema migration — add `credential_id`/`user_id` to `Order`/`Fill`/`Position`/`StrategyRun`), multiple `backend/execution/*` files (credential-scoping throughout), `backend/worker/runner.py` (single-account → multi-account process model), `api/routers/quick_trade.py` | Small: `api/routers/quick_trade.py` only, extending `api/models.py`'s already-multi-tenant schema (e.g. its existing `Trade` table, whose `strategy_id` design already anticipates a non-strategy caller per its own in-code comment) |
...
| Fill/Trade persistence for quick-trade orders | **Order Lifecycle Domain으로 새로 정의** (extending `api/models.py`'s existing `Trade` table, per its own already-nullable `strategy_id` design intent) |
...
Design (not implement) the new, quick-trade-scoped lightweight domain identified in §9: a `RiskManager`-gated, idempotency-checked, `Trade`-table-persisted order path for `api/routers/quick_trade.py`, explicitly independent of `backend/execution/*`'s schema and process model. Must also resolve the `os.environ` credential-mutation concurrency risk found in §1 as part of the same design pass, since any hardening of this path should not build on top of an unsafe credential-injection mechanism.

---

## Final Review Summary

- **Quick Trade 실제 런타임 경로**: 완전히 상태를 갖지 않는(stateless) 요청 단위 어댑터 — 브로커 호출 1회, 응답 즉시 반환, 어떤 상태/체결/포지션도 추적·영속화하지 않음. `_load_kis`가 요청마다 프로세스 전역 `os.environ`을 변경하는 잠재적 동시성 위험도 신규 확인됨.
