...
> **P5-02E implementation note (2026-07-18):** this document's "Migration order" section below proposes a 2-PR split (an additive PR with a live dual-app differential test, then a separate cutover PR) specifically to keep a CodeRabbit-raised concern satisfied: the equivalence proof must still be able to run in the exact commit that ships, not just at some intermediate state. P5-02E executed as a **single PR** instead, closing that same gap with an **analytically-derived golden-value test** (`api/tests/test_compat_unified.py::TestTransformGolden`) rather than a live dual-app comparison: expected output is hand-computed by applying the documented `_PathConfig` algorithm to hardcoded canned input, in a DB-free stub app running only the new `CompatMiddleware`. Because it never imports the old classes, it keeps compiling and gating CI in the same commit that deletes them — achieving the same "proof survives the final shipped state" property this section was written to guarantee, without a transient second app or a two-PR split. See `docs/P5_COMPAT_REFACTOR.md` for the executed design.
