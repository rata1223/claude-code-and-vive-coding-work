...
> **Correction to `docs/P5_ORDERS_COMPAT_AUDIT.md`'s framing:** the audit correctly captured `getBalance(credentialId, marketType='spot')`'s default value, but its later analysis (and this task's original test naming) implicitly treated `market_type` as carrying `"us"`/`"kr"` values. Verified against the live UI (`frontend/src/stores/index.js:299`, `frontend/src/views/quick-trade/index.vue:200-201`): `marketType` is actually a **crypto-exchange concept** — `"spot"` or `"swap"` — not a KIS equity-market selector. Neither value ever equals `"kr"` (`api/routers/quick_trade.py`'s `if market.lower() == "kr"` check), so the `market_type`→`market` rename is still a correct, harmless, necessary DTO-level fix (it stops silently dropping the parameter), but it does **not** achieve real market-selection interop today — every call from the live UI resolves to the same US-equity branch it did before this fix, regardless of which toggle the user picks. This reflects a deeper, pre-existing mismatch (a crypto-exchange-shaped Quick Trade screen sitting on top of a KIS-equities-only backend) that `CLAUDE.md`'s own Stage 7 roadmap ("Mobile 브로커 UI 교체") already earmarks for a future frontend replacement — out of scope for this adapter-only phase.
