...
Pending is subtracted **before** the quantity is chosen, so "close everything" means everything *net* sellable. Resolving it afterwards made a close-all pick the broker figure and then fail its own check whenever a reservation existed — 10 sellable with 4 reserved refused the close instead of closing the 6 that really were sellable. An explicit over-ask is still rejected outright, never clamped.

A caller-supplied **replay short-circuits before any broker lookup**. This handler derives its quantity from the live figure, so re-running the resolution would judge the retry against a figure the first submission already reduced and reject it. The short-circuit still enforces the conflict check it bypasses: `credential_id`, symbol, market and (when given) qty must match the stored row, otherwise the caller gets `Duplicate idempotency key with different parameters` rather than an unrelated order. `broker_nets_pending=True` is available for a broker whose orderable figure already excludes resting orders, so it is not subtracted twice.
