...
The close-all quantity is deliberately **independent of local pending state** — it is whatever the *broker* says is sellable. Netting pending into it was tried and reverted: the server-derived idempotency key is a function of that quantity, so letting local state move it means two identical close-all clicks derive different keys, dedup never fires, and the position is sold twice. A close refused while one of our own sells is still in flight is the safe outcome; a duplicate order is not. Pending is applied after the key is fixed. An explicit over-ask is rejected outright, never clamped.
