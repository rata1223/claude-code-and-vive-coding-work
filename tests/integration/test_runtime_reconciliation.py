...
def _register(poller, order: BOrder, on_filled):
...
class TestTerminalStateSync:
    ...
    duplicate delivery. Exercises the worker's real _make_terminal_callback."""

    def _worker_terminal_cb(self, tracker, machine):
        from backend.worker.runner import StrategyWorker
        worker = object.__new__(StrategyWorker)   # bare — skip heavy __init__
        return worker._make_terminal_callback(tracker, machine)
