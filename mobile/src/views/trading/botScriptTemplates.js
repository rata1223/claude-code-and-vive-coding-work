...
/**
 * 根据 AI 返回的 bot_recommend 结构构建与 BotForm.buildPayload 一致的创建机器人请求体
 */
export function buildBotPayloadFromRecommendation (rec, { credentialId }) {
  if (!rec) throw new Error('invalid recommendation')
  const defaults = DEFAULT_BOT_PARAMS()
  const botType = defaults[rec.botType] ? rec.botType : 'grid'
  const base = rec.baseConfig || {}
  const risk = rec.riskConfig || {}
  const aiParams = rec.strategyParams || {}

  const symbol = base.symbol || ''
  const timeframe = base.timeframe || '1h'
  const marketType = base.marketType || 'swap'
  const leverage = marketType === 'spot' ? 1 : (Number(base.leverage) || 5)
  const capital = Number(base.initialCapital) || 1000

  const sp = { ...defaults[botType] }
  Object.entries(aiParams).forEach(([k, v]) => {
    if (v != null && k in sp) sp[k] = v
  })

  const scriptParams = { ...sp }
  if (capital > 0) scriptParams._initialCapital = capital
  if (botType === 'grid') {
    scriptParams.amountPerGrid = scriptParams.amountPerGrid || (capital / (scriptParams.gridCount || 20))
  } else if (botType === 'martingale') {
    scriptParams.initialAmount = scriptParams.initialAmount || Math.max(10, capital / ((Math.pow(scriptParams.multiplier || 2, scriptParams.maxLayers || 5) - 1) / ((scriptParams.multiplier || 2) - 1)))
  } else if (botType === 'dca') {
    scriptParams.totalBudget = scriptParams.totalBudget || capital
  }

  const isGridOrMartingale = botType === 'grid' || botType === 'martingale'
  const effectiveTimeframe = isGridOrMartingale ? '1m' : timeframe
  const code = (rec.strategy_code && String(rec.strategy_code).trim())
    || generateBotScript(botType, scriptParams, { timeframe: effectiveTimeframe })

  const stopLossPct = botType === 'martingale' ? 0 : Number(risk.stopLossPct ?? 10)
  const takeProfitPct = botType === 'martingale' ? 0 : Number(risk.takeProfitPct ?? 20)
  const maxPosition = botType === 'martingale' ? 0 : Number(risk.maxPosition ?? 0)

  const botName = rec.botName || `${botType.toUpperCase()} · ${symbol}`

  return {
    strategy_name: botName,
    strategy_type: 'ScriptStrategy',
    strategy_mode: 'bot',
    strategy_code: code,
    market_category: 'Crypto',
    execution_mode: 'live',
    exchange_config: { credential_id: credentialId },
    trading_config: {
      symbol,
      ...
      bot_type: botType,
      bot_params: sp,
      order_mode: (botType === 'martingale' || botType === 'trend') ? 'market' : 'maker',
...
/**
 * 从 /api/strategies/ai-generate 的响应里提取 bot_recommend 结构
 */
export function normalizeAiBotRecommendation (payload, fallback = {}) {
  if (!payload) return null
  if (payload.bot_recommend && typeof payload.bot_recommend === 'object') {
    return normalizeAiBotRecommendation(payload.bot_recommend, fallback)
  }
  if (payload.data && (payload.data.botType || payload.data.strategyParams || payload.data.bot_recommend)) {
    return normalizeAiBotRecommendation(payload.data, fallback)
  }
  if (payload.recommendation) {
    return normalizeAiBotRecommendation(payload.recommendation, fallback)
  }
  const hasStructured =
    payload.botType ||
    payload.bot_type ||
  ...
  const botType = payload.botType || payload.bot_type || 'grid'
  const baseConfig = payload.baseConfig || payload.base_config || {
    symbol: payload.symbol || fallback.symbol,
    timeframe: payload.timeframe || fallback.timeframe,
    marketType: payload.marketType || payload.market_type || fallback.marketType,
    leverage: payload.leverage,
    initialCapital: payload.initialCapital || payload.initial_capital || fallback.initialCapital
  }
  const riskConfig = payload.riskConfig || payload.risk_config || {
    stopLossPct: payload.stopLossPct || payload.stop_loss_pct,
    takeProfitPct: payload.takeProfitPct || payload.take_profit_pct,
    maxPosition: payload.maxPosition || payload.max_position
  }
  return {
    botType,
    botName: payload.botName || payload.strategyName || payload.strategy_name || '',
    reason: payload.reason || payload.analysis || payload.ai_reason || payload.summary || '',
    baseConfig,
    riskConfig,
    strategyParams: payload.strategyParams || payload.strategy_params || {}
  }
}
