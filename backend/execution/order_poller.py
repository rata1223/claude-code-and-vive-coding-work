...
class OrderFillPoller:
    ...
    def register(
        ...
        entry = _PollEntry(
            ...
            # Seed the replay high-water mark from the order's already-known filled
            # quantity. For live orders this is 0; for orders re-registered after a
            # restart it is the DB-persisted filled_qty, so a re-poll never re-reports
            # shares that were already processed before the crash (persistent watermark).
            last_reported_qty=order.filled_qty or 0,
        )
        with self._lock:
